...
/* VARIABLES */
int16_t uart_dma_buff[MIC_BUFF_SIZE / 2] = {0};
...
void log_data(int16_t *data) {
    if (!uart_dma_transfer_done) return; // First wait for previous transfer to complete
    uint16_t idx = 0;
    for (int i = 0; i < MIC_BUFF_SIZE; i+=2) {
        uart_dma_buff[idx++] = data[i];
    }
    transmit((uint8_t *) uart_dma_buff, 2 * idx);
}
